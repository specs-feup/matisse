 ...
 *
 */
public class EndWhileSt extends ReservedWordSt {

    EndWhileSt(StatementData data, Collection<MatlabNode> children) {
        super(data, children);
    }

    EndWhileSt(int lineNumber) {
        super(new StatementData(lineNumber, true),
                Arrays.asList(MatlabNodeFactory.newReservedWord(ReservedWord.EndWhile)));
    }

    @Override
    protected MatlabNode copyPrivate() {
        return new EndWhileSt(getData(), Collections.emptyList());
    }

    @Override
    public ReservedWord getReservedWord() {
        return ReservedWord.EndWhile;
    }
