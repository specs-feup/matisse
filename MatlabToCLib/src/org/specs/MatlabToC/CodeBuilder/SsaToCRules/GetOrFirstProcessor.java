...
public class GetOrFirstProcessor implements SsaToCRule {

    @Override
    public boolean accepts(SsaToCBuilderService builder, SsaInstruction instruction) {
	return instruction instanceof GetOrFirstInstruction;
    }
	...
	if (inputType instanceof MatrixType) {
	    ...
	    emitMatrixGetImplementation(builder, currentBlock, output, matrix, index);
	...
	CNode indexValue = builder.generateVariableExpressionForSsaName(currentBlock, index, false);

	CNode zeroNode = CNodeFactory.newCNumber(0);
	CNode oneNode = CNodeFactory.newCNumber(1);

	CNode normalZeroBasedIndex = COperator.Subtraction
