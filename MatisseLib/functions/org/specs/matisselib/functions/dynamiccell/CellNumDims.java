...
public class CellNumDims extends AInstanceBuilder {

    private CellNumDims(ProviderData data) {
	super(data);
    }
	...
	return new GenericInstanceProvider(new MatisseInputsChecker()
		.numOfInputs(1)
		.isDynamicCell(0), data -> new CellNumDims(data).create());
    }
	...
	InlineCode code = tokens -> {
	    ...
	    return token.getCodeForLeftSideOf(PrecedenceLevel.MemberAccessThroughPointer) + "->"
		    + DynamicCellStruct.CELL_DIMS;
	};
	InlinedInstance instance = new InlinedInstance(functionType, "$cell_num_dims$" + type.getSmallId(), code);
