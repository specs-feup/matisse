 ...
 *
 */
public class EndFunctionSt extends ReservedWordSt {

    EndFunctionSt(StatementData data, Collection<MatlabNode> children) {
        super(data, children);
    }

    EndFunctionSt(int lineNumber) {
        super(new StatementData(lineNumber, true),
                Arrays.asList(MatlabNodeFactory.newReservedWord(ReservedWord.EndFunction)));
    }

    @Override
    protected MatlabNode copyPrivate() {
        return new EndFunctionSt(getData(), Collections.emptyList());
    }

    @Override
    public ReservedWord getReservedWord() {
        return ReservedWord.EndFunction;
    }
