 ...
 *
 */
public class SpmdSt extends ReservedWordSt {

    SpmdSt(StatementData data, Collection<MatlabNode> children) {
	super(data, children);
    }

    SpmdSt(int lineNumber, List<MatlabNode> children) {
	super(new StatementData(lineNumber, true), children);
    }

    @Override
    protected MatlabNode copyPrivate() {
	return new SpmdSt(getData(), Collections.emptyList());
    }

    @Override
    public ReservedWord getReservedWord() {
	return ReservedWord.Spmd;
    }
