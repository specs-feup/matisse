 ...
 * @author Joao Bispo
 */
public class AppTesterLauncher implements AppDefaultConfig, AppSource {
	...
	AppTesterLauncher app = new AppTesterLauncher();
	...
	gui.setTitle("Application Tester v0.1");
	gui.execute();
    }
	...
	AppTesterData data = null;
	try {
	    data = AppTesterSetup.newData(setupData);
	} catch (Exception e) {
	...
	AppTester appBody = new AppTester(data);
	...
	return new AppTesterLauncher();
    }

    @Override
    public SetupDefinition getEnumKeys() {
	return SetupDefinition.create(AppTesterSetup.class);
    }
