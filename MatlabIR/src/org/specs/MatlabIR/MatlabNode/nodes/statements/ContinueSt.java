 ...
 *
 */
public class ContinueSt extends ReservedWordSt {

    ContinueSt(StatementData data, Collection<MatlabNode> children) {
	super(data, children);
    }

    ContinueSt(int lineNumber, boolean display, List<MatlabNode> children) {
	super(new StatementData(lineNumber, display), children);
    }

    @Override
    protected MatlabNode copyPrivate() {
	return new ContinueSt(getData(), Collections.emptyList());
    }

    @Override
    public ReservedWord getReservedWord() {
	return ReservedWord.Continue;
    }
