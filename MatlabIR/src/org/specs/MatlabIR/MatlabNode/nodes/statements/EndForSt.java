 ...
 *
 */
public class EndForSt extends ReservedWordSt {

    EndForSt(StatementData data, Collection<MatlabNode> children) {
        super(data, children);
    }

    EndForSt(int lineNumber) {
        super(new StatementData(lineNumber, true),
                Arrays.asList(MatlabNodeFactory.newReservedWord(ReservedWord.EndFor)));
    }

    @Override
    protected MatlabNode copyPrivate() {
        return new EndForSt(getData(), Collections.emptyList());
    }

    @Override
    public ReservedWord getReservedWord() {
        return ReservedWord.EndFor;
    }
