 ...
 *
 */
public class EndSt extends ReservedWordSt {

    EndSt(StatementData data, Collection<MatlabNode> children) {
	super(data, children);
    }

    EndSt(int lineNumber) {
	super(new StatementData(lineNumber, true), Arrays.asList(MatlabNodeFactory.newReservedWord(ReservedWord.End)));
    }

    @Override
    protected MatlabNode copyPrivate() {
	return new EndSt(getData(), Collections.emptyList());
    }
