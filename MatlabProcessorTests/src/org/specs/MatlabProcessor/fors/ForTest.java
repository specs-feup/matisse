...
package org.specs.MatlabProcessor.fors;
...
public class ForTest extends TestCase {
	...
	for (ForTestResource resource : ForTestResource.values()) {
	...
	for (ForErrorResource resource : ForErrorResource.values()) {
	...
	for (StatementNode statement : block.getChildren(StatementNode.class)) {
		...
		ForSt forStatement = (ForSt) statement;

		output.append("Loop Variable: ");
		output.append(forStatement.getIndexVar().getName());
		output.append("\nExpression: ");
		output.append(forStatement.getExpression().getCode());
		output.append("\nBody: ");
	    } else {
