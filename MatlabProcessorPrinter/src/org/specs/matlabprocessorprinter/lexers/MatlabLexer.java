...
public class MatlabLexer extends SimpleRegexLexer {
    ...
    static {
        ...
        KEYWORDS.add("end(function|if|for|while)?");
        KEYWORDS.add("for");
        ...
        PROPERTIES.put(TokenType.COMMENT, "[%#].*");
        PROPERTIES.put(TokenType.NUMBER, "[0-9]+(\\.[0-9]+)?");
        PROPERTIES.put(TokenType.KEYWORD, KEYWORDS.stream().collect(Collectors.joining("|")));
        PROPERTIES.put(TokenType.KEYWORD2, BUILTINS.stream().collect(Collectors.joining("|")));
        PROPERTIES.put(TokenType.IDENTIFIER, "[a-zA-Z][a-zA-Z0-9_]*");
        PROPERTIES.put(TokenType.STRING, "'.*?'");
        PROPERTIES.put(TokenType.STRING2, "\".*?\"");
    }
