...
public class RelativeGetTests {
    @Test
    public void testDynamicRelativeGet1D() {
        MockSsaToCBuilderService builder = makeBuilder();

        RelativeGetInstruction get = new RelativeGetInstruction("$out", "$in", "$rel", Arrays.asList("i$1"));
        RelativeGetProcessor processor = new RelativeGetProcessor();

        TestUtils.test(builder, get, processor, "out = in->data[in->length == 1 ? 0 : i1 - 1];");
    }

    @Test
    public void testDynamicRelativeGet2D() {
        MockSsaToCBuilderService builder = makeBuilder();

        RelativeGetInstruction get = new RelativeGetInstruction("$out", "$in", "$rel", Arrays.asList("i$1", "i$2"));
        RelativeGetProcessor processor = new RelativeGetProcessor();

        TestUtils.test(builder, get, processor,
                "out = in->data[in->length == 1 ? 0 : i1 - 1 + (i2 - 1) * rel->data[0]];");
    }
    ...
    private static MockSsaToCBuilderService makeBuilder() {
        ...
        final VariableManager manager = new MockVariableManager() {
            ...
            public Optional<VariableType> getVariableTypeFromFinalName(String finalName) {
                ...
                case "rel":
                    return Optional.of(intMatrixType);
                case "i1":
                case "i2":
