...
public class WriteGlobalInstructionRule implements TypeInferenceRule {

    @Override
    public boolean accepts(SsaInstruction instruction) {
        return instruction instanceof WriteGlobalInstruction;
    }

    @Override
    public void inferTypes(TypeInferenceContext context, InstructionLocation location, SsaInstruction instruction) {
        WriteGlobalInstruction writeGlobal = (WriteGlobalInstruction) instruction;

        GlobalTypeProvider globalTypeProvider = context.getPassData()
                .get(ProjectPassServices.GLOBAL_TYPE_PROVIDER);

        String global = writeGlobal.getGlobal().substring(1);

        Optional<VariableType> variableType = globalTypeProvider.get(global);
        if (!variableType.isPresent()) {
            throw context.getInstructionReportService()
                    .emitError(context.getInstance(), instruction, PassMessage.TYPE_INFERENCE_FAILURE,
                            "Missing type of global " + global + ".");
        }
    }
