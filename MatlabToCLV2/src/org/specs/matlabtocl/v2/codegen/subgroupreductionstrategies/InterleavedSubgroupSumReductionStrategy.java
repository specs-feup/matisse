...
public class InterleavedSubgroupSumReductionStrategy extends ReductionStrategy {
    @Override
    public CInstructionList buildReductionOperation(ReductionComputation reductionComputation,
        ...
        boolean nvidiaFallback = builder.getCurrentProvider().getSettings()
        ...
        CNode localIdNode = builder.getLocalIdNodes().get(0);
        int subGroupSize = builder.getCurrentProvider().getSettings().get(MatisseCLKeys.SUB_GROUP_SIZE);
        int subGroupSizeLog2 = getLog2(subGroupSize);
        CNode subGroupSizeLog2Node = CNodeFactory.newCNumber(subGroupSizeLog2);

        CNode stopExpr = FunctionInstanceUtils.getFunctionCall(CLBinaryOperator.LESS_THAN,
                providerData,
                CNodeFactory.newVariable(tmp),
                subGroupSizeLog2Node);
        ...
        block.addLiteralInstruction(builder.getSubGroupBarrierCode());
